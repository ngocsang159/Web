...
namespace SmartPhone.Services
{
    ...
    {
        public bool AddOrUpdate(Product product, long CategoryId, List<ProductColor> productColors, List<ProductImage> productImages)
        {
            if (product.Id == 0)
            {
        ...
        public ResponseDataAdmin<Product> GetListProductAll(long CategoryId, int OrderBy, int PageNumber, int PageSize)
        {
            ...
            if(CategoryId == 0)
            {
                data = db.Products.Where(n => n.Status == 1 && n.IsDelete == false).OrderBy(n => n.Position).Skip(PageSize * (PageNumber - 1)).Take(PageSize).ToList();
                total = db.Products.Where(n => n.Status == 1 && n.IsDelete == false).Count();
            }
            else
            {
            ...
            {
                data = data.OrderBy(n => n.PriceSale).ToList();
            }
            ResponseDataAdmin<Product> dataProduct = new ResponseDataAdmin<Product>();
            dataProduct.data = data;
            dataProduct.pageNumber = PageNumber;
